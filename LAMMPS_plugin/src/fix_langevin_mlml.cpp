...
FixLangevinMLML::FixLangevinMLML(LAMMPS *lmp, int narg, char **arg) :
...
{
  if (narg < 8) error->all(FLERR, "Illegal fix langevin/mlml command");
  ...
  pot_for_eval = utils::inumeric(FLERR, arg[7], false, lmp);
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "angmom") == 0) {
      error->all(FLERR, "angmom not supported in fix langevin/mlml");
      ...
      error->all(FLERR, "gjf not supported in fix langevin/mlml");
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix langevin command");
      if (strcmp(arg[iarg + 1], "no") == 0) {
        gjfflag = 0;
        osflag = 0;
      ...
      error->all(FLERR, "omega not supported in fix langevin/mlml");
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix langevin command");
      ...
      error->all(FLERR, "tally not supported in fix langevin/mlml");
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix langevin command");
      tallyflag = utils::logical(FLERR, arg[iarg + 1], false, lmp);
      iarg += 2;
    } else if (strcmp(arg[iarg], "zero") == 0) {
      error->all(FLERR, "zero not supported in fix langevin/mlml");
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix langevin command");
  ...
  if (gjfflag) {
    FixLangevinMLML::grow_arrays(atom->nmax);
...
FixLangevinMLML::~FixLangevinMLML()
{
  delete random;
  delete[] tstr;
  delete[] gfactor1;
...
void FixLangevinMLML::setup(int vflag)
{
  if (gjfflag) {
    double dtfm;
    double dt = update->dt;
...
void FixLangevinMLML::initial_integrate(int /* vflag */)
{
  double **v = atom->v;
  double **f = atom->f;
  int *mask = atom->mask;
...
void FixLangevinMLML::post_force(int /*vflag*/)
{
  double *rmass = atom->rmass;
...
void FixLangevinMLML::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixLangevinMLML::post_force_templated()
{
  ...
  if (d2_eval==nullptr) error->all(FLERR,"Fix langevin/mlml requires d2_eval set by fix mlml");
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      f[i][0] += (fdrag[0] + fran[0])*d2_eval[i][pot_for_eval-1];
      f[i][1] += (fdrag[1] + fran[1])*d2_eval[i][pot_for_eval-1];
      f[i][2] += (fdrag[2] + fran[2])*d2_eval[i][pot_for_eval-1];
...
void FixLangevinMLML::compute_target()
{
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinMLML::omega_thermostat()
{
  double gamma1,gamma2;
...
void FixLangevinMLML::angmom_thermostat()
{
  double gamma1,gamma2;
...
void FixLangevinMLML::end_of_step()
{
  double **v = atom->v;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinMLML::reset_target(double t_new)
{
...
int FixLangevinMLML::modify_param(int narg, char **arg)
{
...
double FixLangevinMLML::compute_scalar()
{
...
void *FixLangevinMLML::extract(const char *str, int &dim)
{
  dim = 0;
...
double FixLangevinMLML::memory_usage()
{
  double bytes = 0.0;
...
void FixLangevinMLML::grow_arrays(int nmax)
{
...
void FixLangevinMLML::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixLangevinMLML::pack_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = franprev[i][0];
  buf[n++] = franprev[i][1];
...
int FixLangevinMLML::unpack_exchange(int nlocal, double *buf)
{
  int n = 0;
